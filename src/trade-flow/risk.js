...
import { tfSpreadBracket, tfUpdateIntradayRMult, tfComputeIntradayRiskSize, tfUpdateIntradaySizing } from './intraday-sizing.js';
import { tfRefreshHeaderOnly } from './stepper.js';
import { tfUpdateSwingSizing } from './swing-sizing.js';

export function tfMoneyText(value) {
  const n = Number(value);
  return Number.isFinite(n) && n > 0 ? `$${n.toFixed(2)}` : '—';
}

export function tfPctText(value, digits = 1) {
  const n = Number(value);
  return Number.isFinite(n) ? `${n.toFixed(digits)}%` : '—';
}

export function tfSignedMoneyText(value, digits = 0) {
  const n = Number(value);
...
export function tfSignedPctText(value, digits = 1) {
  const n = Number(value);
  if (!Number.isFinite(n)) return '—';
  return `${n >= 0 ? '+' : ''}${n.toFixed(digits)}%`;
}

export function tfAbsMoneyText(value, digits = 0) {
  const n = Number(value);
...
export function tfSpreadReadHtml(spread, max = null) {
...
export function tfRiskDirection({ entry, stop, target } = {}) {
...
export function tfRiskLevelStat({ entry, price, qty, mult = 1, rBase = null, direction = 1 } = {}) {
  const e = Number(entry);
...
export function tfRiskLevelRows({ entry, stop, target, qty, mult = 1 } = {}) {
  const e = Number(entry);
  const s = Number(stop);
  const t = Number(target);
  const q = Number(qty);
  ...
  const direction = tfRiskDirection({ entry: e, stop: s, target: t });
  ...
  const make = (label, price, cls) => {
    const p = Number(price);
    const stat = tfRiskLevelStat({ entry: e, price: p, qty: q, mult: m, rBase, direction });
...
export function tfRiskRailHtml({ entry, stop, target, qty, mult = 1 } = {}) {
  const e = Number(entry);
  const s = Number(stop);
  const t = Number(target);
  const q = Number(qty);
  ...
  const direction = tfRiskDirection({ entry: e, stop: s, target: t });
  const stopStat = tfRiskLevelStat({ entry: e, price: s, qty: q, mult: m, rBase: loss, direction });
  const targetStat = tfRiskLevelStat({ entry: e, price: t, qty: q, mult: m, rBase: loss, direction });
  const stopPnlText = stopStat
    ? `${tfSignedMoneyText(stopStat.pnl, 2)} (${tfSignedPctText(stopStat.pct)})`
    : `-${tfAbsMoneyText(loss, 2)}`;
  const targetPnlText = targetStat
    ? `${tfSignedMoneyText(targetStat.pnl, 2)} (${tfSignedPctText(targetStat.pct)})`
    : tfSignedMoneyText(reward, 2);
  ...
  const zone = (cls, widthPct, caption, price, pnlText, rLabel) => `
    <div class="tf-risk-zone ${cls}" style="width:${widthPct.toFixed(2)}%;">
      ...
      <strong>${tfMoneyText(price)}</strong>
      <span>${pnlText}</span>
    </div>`;
  return `
    <div class="tf-risk-rail">
      ...
      <div class="tf-risk-entry-marker" style="left:${lossPct.toFixed(2)}%;"><span class="tf-risk-entry-label">ENTRY ${tfMoneyText(e)}</span></div>
    </div>`;
}
...
export function tfPriceSliderBounds({ entry, stop, target, kind } = {}) {
  const e = Number(entry);
  const s = Number(stop);
  const t = Number(target);
  if (!(e > 0)) return null;
  const dir = tfRiskDirection({ entry: e, stop: s, target: t });
  const tick = 0.01;
...
function tfPriceSliderSummaryHtml({ entry, price, qty, mult, rBase, direction } = {}) {
  const stat = tfRiskLevelStat({ entry, price, qty, mult, rBase, direction });
  if (!stat) return '<span>—</span>';
  const cls = stat.pnl < 0 ? 'neg' : stat.pnl > 0 ? 'pos' : '';
  return `<span class="${cls}">${tfSignedMoneyText(stat.pnl, 0)} · ${stat.pct >= 0 ? '+' : ''}${stat.pct.toFixed(1)}% · ${stat.r >= 0 ? '+' : ''}${stat.r.toFixed(2)}R</span>`;
}

export function tfRenderPriceLevelSlidersHtml({ entry, stop, target, qty, mult = 1 } = {}) {
  const e = Number(entry);
  const s = Number(stop);
  const t = Number(target);
  const q = Number(qty);
  ...
  const row = (kind, label, value) => {
    const bounds = tfPriceSliderBounds({ entry: e, stop: s, target: t, kind });
    if (!bounds) return '';
    ...
    return `
      <div class="tf-price-adjust-row" data-tf-price-row="${kind}">
        <div class="tf-price-adjust-head">
          <span>${label}</span>
          <output data-tf-price-output="${kind}">${tfMoneyText(clamped)}</output>
        </div>
...
export function tfRenderRiskProfileHtml({ entry, stop, target, qty, mult = 1 } = {}) {
  const e = Number(entry);
  const s = Number(stop);
  const t = Number(target);
  const q = Number(qty);
  ...
  return `
    <div class="tf-risk-profile" data-tf-risk-entry="${e}" data-tf-risk-stop="${s}" data-tf-risk-target="${t}" data-tf-risk-qty="${q}" data-tf-risk-mult="${m}" data-tf-risk-unit="${loss}">
      <div data-tf-risk-rail-wrap>${tfRiskRailHtml({ entry, stop, target, qty, mult })}</div>
      ${tfRenderPriceLevelSlidersHtml({ entry, stop, target, qty, mult })}
    </div>`;
}

export function tfRiskArgsFromProfile(profile) {
...
export function tfSetPriceLevel(kind, raw) {
  ...
  if (mode === 'intraday') {
    ...
    if (typeof tfUpdateIntradayRMult === 'function') tfUpdateIntradayRMult();
  } else {
  ...
  saveState();
  if (typeof tfRefreshHeaderOnly === 'function') tfRefreshHeaderOnly();
}

export function tfRefreshPriceSliderReadout(slider) {
  ...
  const args = tfRiskArgsFromProfile(profile);
  ...
  let mult = args.mult;
  const mode = (state.tradeFlow && state.tradeFlow.mode) || 'swing';
  if (mode === 'intraday' && typeof tfComputeIntradayRiskSize === 'function') {
  ...
  if (output) output.textContent = tfMoneyText(price);
  ['stop', 'target'].forEach(rowKind => {
    ...
    if (rowOutput) rowOutput.textContent = tfMoneyText(rowPrice);
  });
  const railWrap = profile.querySelector('[data-tf-risk-rail-wrap]');
  if (railWrap) railWrap.innerHTML = tfRiskRailHtml({ entry: args.entry, stop, target, qty, mult });
}

export function tfBindPriceLevelSliders() {
  document.querySelectorAll('#panel-trade [data-tf-price-slider]').forEach(slider => {
    ...
    slider.addEventListener('input', e => {
      ...
      tfSetPriceLevel(kind, e.target.value);
      tfRefreshPriceSliderReadout(e.target);
      ...
      const mode = (state.tradeFlow && state.tradeFlow.mode) || 'swing';
      if (mode === 'intraday' && typeof tfUpdateIntradayRMult === 'function') {
        tfUpdateIntradayRMult();
      }
    });
    slider.addEventListener('change', () => {
      const mode = (state.tradeFlow && state.tradeFlow.mode) || 'swing';
      if (mode === 'intraday' && typeof tfUpdateIntradaySizing === 'function') {
        tfUpdateIntradaySizing();
      } else if (typeof tfUpdateSwingSizing === 'function') {
        tfUpdateSwingSizing();
      }
    });
  });
